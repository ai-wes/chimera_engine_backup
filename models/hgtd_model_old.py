...
from .diffusion_decoder_old import KnowledgeConditionedEnhancedDiffusion
from models.knowledge_guided_edn_old import KGEDN
...
class EnhancedHGTD(nn.Module):
    ...
    def critic_losses(self, z_real, z_fake, local_real=None, global_real=None):
        """
        Compute local and global critic losses for adversarial training.
    ...
    def decode_modality(self, z, modality):
        ...
        if self.use_pathway_tokenizer and hasattr(self, '_pathway_tokens'):
            ...
            if modality in decoded_outputs:
                # Handle variant output which is a dict
                if modality == 'variant' and isinstance(decoded_outputs[modality], dict):
                    # Convert variant dict to tensor representation
                    positions = decoded_outputs[modality]['positions']
                    alleles = decoded_outputs[modality]['alleles']
                    # Combine positions and alleles into a single tensor
                    # Use positions as the main output (most informative)
                    return positions
                # Handle clinical output which is also a dict
                elif modality == 'clinical' and isinstance(decoded_outputs[modality], dict):
