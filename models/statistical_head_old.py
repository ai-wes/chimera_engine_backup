...
class StatisticalHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor, target_stats: dict, diversity_scale: float = None) -> torch.Tensor:
        ...
        if self.training:
            # During training, we sample to explore and provide stochasticity
            # This is the reparameterization trick
            epsilon = torch.randn_like(final_std)
            output = final_mean_with_diversity + epsilon * final_std
        else:
            # During evaluation, we use the mean for deterministic, stable output
            output = final_mean
        
        # Apply distribution-specific final activation with numerical stability
        if self.dist_type == 'log_normal':
            # Gene expression is often log-normally distributed. exp() achieves this.
            # Clamp to prevent numerical overflow
            output_clamped = torch.clamp(output, min=-10, max=10)
            return torch.exp(output_clamped)
        elif self.dist_type == 'beta':
            # Methylation beta-values are in [0, 1]. sigmoid() achieves this.
            # Clamp to prevent extreme values
            output_clamped = torch.clamp(output, min=-10, max=10)
            return torch.sigmoid(output_clamped)
        else: # 'normal'
            # Clamp to reasonable range
            return torch.clamp(output, min=-50, max=50)